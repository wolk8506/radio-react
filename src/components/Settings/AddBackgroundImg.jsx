...
import { getThemeWalpaper } from 'store/root/selectors';
import { setThemeWalpaper } from 'store/root/actions';
import { fileActions } from 'store/files/actions';
import { fileSelector } from 'store/files/selectors';
import { deleteFile, uploadWalpaper } from 'store/files/operations';
...
const { ElementImg } = require('../Elements');

export const AddBackgroundImg = ({ value, onHandleChange }) => {
  const dispatch = useDispatch();
  const BACKGROUND = useSelector(getThemeWalpaper);
  ...
  const handleSaveImg = () => {
    dispatch(deleteFile([BACKGROUND]));
    const formData = new FormData();
    formData.append('file', backgroundImg.file);

    dispatch(
      uploadWalpaper(formData, {
        headers: {
  ...
  useEffect(() => {
    console.log('statusUploadWalpaper', statusUploadWalpaper);

    if (statusUploadWalpaper) {
      console.log(backgroundImg.imgNewURL);
      dispatch(fileActions.setStatusUploadWalpaper());
      backgroundImg.imgNewURL && dispatch(setThemeWalpaper(backgroundImg.imgNewURL));
